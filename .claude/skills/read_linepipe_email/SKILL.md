...
当邮件提供多个选项时，**默认生成所有组合的笛卡尔积**：

| 场景 | 示例 | 生成行数 |
|------|------|----------|
| 多种壁厚 + 多种长度 | 2 种壁厚 × 3 种长度 | 6 行 |
| 多种尺寸 + 多种壁厚 | 3 种尺寸 × 2 种壁厚 | 6 行 |
| 单一选项 | 1 种壁厚 × 1 种长度 | 1 行 |

**数量分配规则**：
- 总长度已知时：按各长度比例分配（如 42600ft ÷ 3 种长度）
- 总数量已知时：按组合数量平均分配
- 单项数量明确时：直接使用

**示例**：邮件要求 "Option #1 STD, Option #2 XS" + "60ft/40ft/30ft 三种长度" + "总长 42600ft"
→ 生成 6 行（2 壁厚 × 3 长度），每种长度 14200ft，数量按长度计算
